/**
 * Returns the current user's stakeholder-map submission for the given activity, if any.
 */
...
export async function GET(request: Request) {
  ...
  const res = await fetch(
    `${base}/api/stakeholder-map-submissions?where[activity][equals]=${activityId}&where[submittedBy][equals]=${userId}&limit=1&depth=0`,
