/**
 * Returns the current user's image-choice response for the given assessment, if any.
 */
...
export async function GET(request: Request) {
  ...
  if (!assessmentId) return NextResponse.json(apiError('assessment query required'), { status: 400 })
  ...
  const res = await fetch(
    `${base}/api/image-choice-responses?where[assessment][equals]=${assessmentId}&where[user][equals]=${userId}&limit=1&depth=0`,
