...
export async function POST(request: Request) {
  ...
  if (!activityId || !Array.isArray(allocations)) {
    return NextResponse.json({ error: 'activityId and allocations (array) required' }, { status: 400 })
  }
  ...
  if (!activityRes.ok) {
    if (activityRes.status === 404) return NextResponse.json({ error: 'Activity not found' }, { status: 404 })
  ...
  if (allocations.length !== audienceCount) {
    return NextResponse.json({ error: 'Allocation count does not match activity audiences' }, { status: 400 })
  }
  ...
  if (!result.canSubmit) {
    ...
    return NextResponse.json({ error: messages.join(' ') || 'Invalid allocation' }, { status: 400 })
  }

  const submissionsRes = await fetch(
    `${CMS_URL}/api/audience-poker-submissions?where[activity][equals]=${activityId}&where[user][equals]=${userId}&limit=1`,
  ...
  if (submissionsData.docs?.length > 0) {
    return NextResponse.json({ error: 'You have already submitted this activity.' }, { status: 409 })
  }
  ...
  if (!createRes.ok) {
    return NextResponse.json(
      { error: (createData as { errors?: { message?: string }[] })?.errors?.[0]?.message ?? 'Failed to save submission' },
