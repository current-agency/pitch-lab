/**
 * Returns the current user's platform-survey response for the given company, if any.
 */
...
export async function GET(request: Request) {
  ...
  if (!companyId) return NextResponse.json(apiError('company query required'), { status: 400 })
  ...
  const res = await fetch(
    `${base}/api/platform-survey-responses?where[company][equals]=${companyId}&where[user][equals]=${userId}&limit=1&depth=0`,
